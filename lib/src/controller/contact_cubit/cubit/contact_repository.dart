...
class ContactRepository {
  ...
  Future<ApiResponse> createNewContact(ContactModel contactModel) async {
    ApiResponse response = await ApiHelper().makePostRequest("contact", contactModel.toJson());
    return response;
  }
  Future<ApiResponse> updateContact(ContactModel contactModel) async {
    ApiResponse response = await ApiHelper().makePatchRequest("contact/${contactModel.id}",contactModel.toJson());
    return response;
  }
  Future<ApiResponse> deleteContact(ContactModel contactModel) async {
    ApiResponse response = await ApiHelper().makeDeleteRequest("contact/${contactModel.id}");
    return response;
  }
}
